  ...
  /*  private void CheckDataBase() {
    ...
    private void checkUser(){
        ...
        if(user.length()>0 && pass.length()>0){
            ...
            retrofit2.Call<String> callback =dataClient.Signin(kh);
            callback.enqueue(new Callback<String>() {
                @Override
                public void onResponse(Call<String> call, Response<String> response) {
                    if(response.isSuccessful()){
                        Toast.makeText(log_in.this, response.body(), Toast.LENGTH_SHORT).show();
                        // move data to destop
                        Intent myIntent = new Intent(log_in.this, destop.class);
                        myIntent.putExtra("username",user);
                        myIntent.putExtra("idUser",response.body());
